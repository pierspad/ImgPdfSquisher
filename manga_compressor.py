...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Compress PDF manga by re-encoding images for a target device',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=f"""
Examples
    # Basic (auto output to ./compressed)
    python {os.path.basename(__file__)} INPUT.pdf +_compressed

    # Custom device and quality
    python {os.path.basename(__file__)} INPUT.pdf OUTPUT.pdf --device tablet_10 --mode auto --quality 75

    # Batch processing
    python {os.path.basename(__file__)} --files a.pdf b.pdf c.pdf --out-dir compressed
    python {os.path.basename(__file__)} --file-list list.txt --suffix _phone
...
Modes
    ...
    parser.add_argument('--max-colors-power', type=int, default=None, metavar='P', help='Palette size as power of two (P). 1=>2, ..., 24=>16,777,216. Overrides --max-colors if given.')
    parser.add_argument('--max-colors', type=int, default=defaults.get('max_colors', DEFAULT_MAX_COLORS), metavar='N', help=f"Maximum colors (legacy). Will be clamped to a power of two in [2..16,777,216]. Default: {defaults.get('max_colors', DEFAULT_MAX_COLORS)}")
    ...
    # NB: help strings in argparse use old-style % formatting; literal % must be escaped as %%
    ...
    if getattr(args, 'max_colors_power', None) is not None:
        ...
        if not 1 <= p <= 24:
            logging.error(f'--max-colors-power must be between 1 and 24 (2^P => [2..16,777,216]), got: {p}')
            sys.exit(1)
        args.max_colors = 2 ** p
    else:
    ...
    compressor = MangaCompressorModular(
        target_device=args.device,
        quality=args.quality,
        max_colors=args.max_colors,
        compression_mode=args.mode,
        workers=args.workers,
        ram_limit_percent=args.ram_limit,
