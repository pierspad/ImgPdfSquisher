...
class MangaCompressorGUI(QWidget):
    ...
    def _build_ui(self):
        ...
        # Riga Modalità: usa un layout orizzontale per avvicinare label e select
        h_mode = QHBoxLayout()
        h_mode.setSpacing(6)
        h_mode.addWidget(self.lbl_mode)
        h_mode.addWidget(self.mode_combo, 1)
        opts_grid.addLayout(h_mode, 1, 0, 1, 2)
    ...
    def _build_language_combo(self):
        ...
        try:
            ...
            for name, emoji, code in sorted(entries, key=lambda x: x[0].lower()):
                label = f"{emoji}  {name}"
